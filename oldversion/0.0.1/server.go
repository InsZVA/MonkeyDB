...
func Handler(conn *net.TCPConn) {
	...
	for {
		buff := tcp.Receive(conn)	//可能发生无限循环->已经ok，本机go编译器有问题
		params := strings.Split(string(buff)," ")
		if params[0] != "auth" {
			tcp.Send(conn,[]byte("Please auth first!"))
			continue
		}
		...
		if int(r.code) == 0 {
			...
			if convert.Equal(passwd,[]byte(params[1])) {
				tcp.Send(conn,[]byte("Auth success"))
				break
			} else {
				tcp.Send(conn,[]byte("Auth fail"))
				continue
			}
		}else {
			break
...
func TranslateMessage(conn *net.TCPConn,db **C.Database,message []byte) {
